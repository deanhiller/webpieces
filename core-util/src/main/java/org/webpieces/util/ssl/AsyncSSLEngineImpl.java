 ...
 *
 */
public class AsyncSSLEngineImpl implements AsyncSSLEngine {

	private static final Logger log = LoggerFactory.getLogger(AsyncSSLEngine.class.getName());
	...
	public AsyncSSLEngineImpl(Object id, SSLEngine sslEngine) {		
		this.id = id;
		...
		log.trace("peerNetData: " + socketToEngineData2.capacity() + 
	...
	public void feedPlainPacket(ByteBuffer b, Object passThrough) {	
		...
		if(log.isTraceEnabled())
			log.trace(id+"feedPlainPacket [in-buffer] pos="+b.position()+" lim="+b.limit());
		...
		if(log.isTraceEnabled())
			log.trace(id+"SSLListener.packetEncrypted pos="+engineToSocketData.position()+
	...
	public PacketAction feedEncryptedPacket(ByteBuffer b, Object passthrough) {
		...
		try {
			...
			if(log.isTraceEnabled()) {
				log.trace(id+"feedEncryptedPacket [in-buffer] pos="+b.position()+
						" lim="+b.limit()+" hsStatus="+hsStatus+" cap="+b.capacity());
			}
			socketToEngineData2.put(b);
			if(log.isTraceEnabled())
				log.trace(id+"[sockToEngine] pos="+socketToEngineData2.position()+" lim="+socketToEngineData2.limit());
	...
	private RuntimeException closeTry(RuntimeException e) {
		try {
			...
			log.warn("Failure trying to shutdown Link properly.  Encryption Engine is closed", ee);
		}
		throw e;
	}
	...
	private PacketAction feedEncryptedPacketImpl(Object passthrough) {
		...
		if(log.isTraceEnabled())
			log.trace(id+"[sockToEngine] finished filling pos="+b.position()+" lim="+b.limit());
		...
		if(log.isTraceEnabled())
			log.trace(id+"[sockToEngine] going to unwrap pos="+b.position()+
		...
		while(b.hasRemaining() && status != Status.BUFFER_UNDERFLOW && status != Status.CLOSED) {
			...
			if(log.isTraceEnabled())
				log.trace(id+"[sockToEngine] unwrap done pos="+b.position()+" lim="+
		...
		if(log.isTraceEnabled())
			log.trace(id+"[sockToEngine] reset pos="+b.position()+" lim="+b.limit()+" status="+status+" hs="+hsStatus);	
		if(log.isTraceEnabled() && isConnected && status != Status.CLOSED) {
			log.trace(id+"[out-buffer] pos="+out.position()+" lim="+out.limit());
		}
	...
	private void resetBuffer(Status status) {
		...
		if(!b.hasRemaining()) {
			...
			if(log.isTraceEnabled())
				log.trace("[sockToEngine] underflow pos="+b.position()+" lim="+b.limit());			
		}
	}
	...
	private void continueHandShake(HandshakeStatus status) {
		...
		case NEED_UNWRAP:				
			if(log.isTraceEnabled())
				log.trace(id+"need unwrap so wait");
			break;
		case NEED_WRAP:
			if(log.isTraceEnabled())
				log.trace(id+"need wrap");
		...
		case NOT_HANDSHAKING:
			if(log.isTraceEnabled())
				log.trace(id+"not handshaking");
			break;
		default:
			log.warn(id+"Bug, should never end up here");
			break;
		}
	}
	...
	protected void runRunnable(Runnable r) {
		...
		try {
			if(log.isTraceEnabled())
				log.trace(id+"Running actual task now");
			
			continueFromTask();
		} catch(IOException e) {
			log.warn(id+"not continuing handshake, exception occurred", e);
			initiateClose();
		} finally {
	...
	private void continueFromTask() throws IOException {
		...
		if(hsStatus == HandshakeStatus.NEED_UNWRAP && b.hasRemaining()) {
			if(log.isTraceEnabled()) {
				log.trace(id+"[Runnable][socketToEngine] refeeding myself pos="+b.position()+" lim="+b.limit());
			}
			feedEncryptedPacketImpl(null);
		} else {
			if(log.isTraceEnabled())
				log.trace(id+"[Runnable]continuing handshake");					
			continueHandShake(hsStatus);
		}
	}
	...
	private void sendHandshakeMessageImpl() throws SSLException {
		if(log.isTraceEnabled())
			log.trace(id+"sending handshake message");
		HELPER.eraseBuffer(empty);

		if(log.isTraceEnabled())
			log.trace(id+"handshake pos="+engineToSocketData.position()+" lim="+engineToSocketData.limit());
		...
		while(hsStatus == HandshakeStatus.NEED_WRAP) {
			...
			HELPER.doneFillingBuffer(engineToSocketData);
			if(log.isTraceEnabled())
				log.trace(id+"write packet pos="+engineToSocketData.position()+" lim="+
		...
		if(log.isTraceEnabled())
			log.trace(id+"status="+hsStatus+" isConn="+isConnected);
	...
	private void fireConnected() {
		if(!isConnected) {
			...
			isConnected = true;
			if(log.isTraceEnabled())
				log.trace(id+"SSLListener.encryptedLinkEstablished");			
			try {
	...
	public synchronized void close() {
		try {
			...
			log.trace(id+"Exception trying to close channel", e);			
		}
	...
	public synchronized void closeImpl() throws IOException {
		...
		if(log.isTraceEnabled())
			log.trace(id+"closing AsynchSSLEngine");
		
		sslEngine.closeOutbound(); //close outbound is like intiating a close
		try {
			initiateCloseImpl(); 
		} catch(IOException e) {
			log.trace(id+"Typical exception as we may be called after farEndClosed", e);
		}
		...
		try {
			...
			log.trace(id+"Normal Expected Exception. Close packet already sent, not waiting for response", e);
		}
	}
	...
	public synchronized void initiateClose() {
		try {
			...
			log.warn(id+"Exception trying to close channel", e);
		}	
	}
	...
	private synchronized void initiateCloseImpl() throws IOException {
		...
		engineToSocketData.clear();
		if(log.isTraceEnabled())
			log.trace(id+"pos1="+engineToAppData.position()+" lim="+engineToSocketData.limit());
		sslEngine.wrap(empty, engineToSocketData);
		if(log.isTraceEnabled())
			log.trace(id+"pos2="+engineToAppData.position()+" lim="+engineToSocketData.limit());
		
		HELPER.doneFillingBuffer(engineToSocketData);
		if(log.isTraceEnabled())
			log.trace(id+"packetEncrypted pos="+engineToSocketData.position()+" lim="+engineToSocketData.limit());
		fireEncryptedPacketToListener(null);
	}
