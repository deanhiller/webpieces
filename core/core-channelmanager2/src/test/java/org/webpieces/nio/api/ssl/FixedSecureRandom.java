...
{
    private static final Logger log = LoggerFactory.getLogger(FixedSecureRandom.class);
            ...
            extends Source
    {
        ...
        {
            this(Hex.parseHexBinary(hexData));
        }

        public BigInteger(int bitLength, String hexData)
        {
            super(expandToBitLength(bitLength, Hex.parseHexBinary(hexData)));
        }
    }

    public FixedSecureRandom(byte[] bytes, String server) {
        this(bytes);
        this.id = server;
    }
    ...
    public void nextBytes(byte[] bytes)
    {
        ...
        String s = Hex.printHexBinary(bytes);
        log.info("hex Bytes generated("+id+")="+s);

        byte[] fixed = Hex.parseHexBinary("AE0FC0FABD0C53866A07F51E9FF6A3773D73E5D508B1DC9449D9E94AC2FA9F96");
        if(fixed.length != bytes.length)
            throw new RuntimeException("not equal, need new bytes");

        System.arraycopy(fixed, _index, bytes, 0, bytes.length);
            ...
            extends SecureRandom
    {
        ...
        byte[] data = Hex.parseHexBinary("01020304ffffffff0506070811111111");
        int    index = 0;
