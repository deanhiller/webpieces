...
public class ProxyHttpStream implements HttpStream {
	...
	private XFuture<StreamWriter> createProxy(XFuture<StreamWriter> future, XFuture<ProxyWriter> futureWriter) {
	...
	private class ProxyResponseStream implements ResponseStream {
		...
		private XFuture<ProxyWriter> futureWriter;
		private Http2Request request;

		public ProxyResponseStream(Http2Request request, ResponseStream stream, XFuture<ProxyWriter> futureWriter) {
...
//			}
	...
	private class ProxyStreamRef implements StreamRef {
		...
		public ProxyStreamRef(XFuture<StreamWriter> writer, StreamRef streamRef) {
			this.writer = writer;
	...
	private class ProxyWriter implements StreamWriter {
		...
		public XFuture<Void> processPiece(StreamMsg data) {
			if(backPressure) {
				...
				return new XFuture<Void>(); //unresolved future will kcik in backpressure AND CPU will stop being used on deregister socket
			}
