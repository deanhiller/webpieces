...
public class Auth0Module implements Module {
    ...
    public Auth0Module(Arguments args, AuthRouteIdSet authRouteIdSet) {
    ...
    public void configure(Binder binder) {
        binder.bind(Auth0Config.class).toInstance(
                new Auth0Config(auth0Domain.get(), auth0ClientId.get(), auth0ClientSecret.get()));

        binder.bind(AuthRouteIdSet.class).toInstance(authRouteIdSet);
    }
